...
def test_is_lost():
    from hangman.round import Round
    solveds = Round(word='')
    tries = solveds.tries = 0
    max_wrong = solveds.max_wrong_tries = 6
    assert solveds.is_lost() is False
    trie = solveds.tries = 7
...
def test_draw_result():
    ...
    if solved.is_word_solved():
        ...
        assert "print('Word is solved, a point goes to you!')" in result
        assert "print('Word is not solved, point goes to your opponent.')"in result
